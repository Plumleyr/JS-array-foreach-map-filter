...
function vowelCount(str){
    ...
    const vowels = 'aeiou';
    Array.from(lowerCaseStr).forEach(letter => {
        if(vowelObj[letter]){
            vowelObj[letter]++;
...
function valTimesIndex(arr){
    const timesIndex = arr.map((num, index) =>{
        return num * index;
    })
    return timesIndex;
}
...
function extractKey(arr, key){
    const extractedKey = arr.map(object =>{
        return object[key];
    })
    return extractedKey;
}
...
function removeVowels(str) {
    let caseSensitiveStrArray = str.toLowerCase().split('');
    let vowels = 'aeiou'
    return (caseSensitiveStrArray.filter(function(word){
...
function doubleOddNumbers(doubled) {
